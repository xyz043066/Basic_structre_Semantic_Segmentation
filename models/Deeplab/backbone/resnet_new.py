...
class ResNet_new(nn.Module):
    ...
    def __init__(self, block, layers, output_stride, BatchNorm, pretrained=True, model_type='resnet-101'):
        ...
        if output_stride == 16:
            ...
            dilations = [1, 1, 1, 2] # 原版 [1 1 1 2]  全设为1表示不添加空洞卷积
        ...
        # self.conv11 = nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1, bias=False)
        # self.bn11 = BatchNorm(64)
        # self.relu11 = nn.ReLU(inplace=True)
        # self.conv22 = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1, bias=False)
        # self.bn22 = BatchNorm(64)
        # self.relu22 = nn.ReLU(inplace=True)

        self.layer1 = self._make_layer(block, 64, layers[0], stride=strides[0], dilation=dilations[0],
                                       BatchNorm=BatchNorm)  # putput [2, 256, 129, 129])
        self.layer2 = self._make_layer(block, 128, layers[1], stride=strides[1], dilation=dilations[1],
                                       BatchNorm=BatchNorm)  # putput [2, 512, 65, 65])
        self.layer3 = self._make_layer(block, 256, layers[2], stride=strides[2], dilation=dilations[2],
                                       BatchNorm=BatchNorm)  # putput [2, 1024, 33, 33])
    ...
    def _load_pretrained_model(self):
        ...
        if self.model_type == 'resnet-101':
            # pretrain_dict = model_zoo.load_url('https://download.pytorch.org/models/resnet101-5d3b4d8f.pth')
            pretrain_dict = model_zoo.load_url('http://sceneparsing.csail.mit.edu/model/pretrained_resnet/resnet101-imagenet.pth')
        elif self.model_type == 'resnet-50':
            # pretrain_dict = model_zoo.load_url('https://download.pytorch.org/models/resnet50-19c8e357.pth')
            pretrain_dict = model_zoo.load_url('http://sceneparsing.csail.mit.edu/model/pretrained_resnet/resnet50-imagenet.pth')
...
# def ResNet18_2(output_stride, BatchNorm, pretrained=True):
